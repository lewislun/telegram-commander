...
export default class ReplyListenerRegistry {
	...
	/** @type {Map.<number, string>} */											fullMsgIdByQueryListenerId = new Map()
	...
	registerCallbackQueryListener(inlineKeyboardMsg, count, targetUserId = undefined, handler) {
		const msgId = this.getFullMsgId(inlineKeyboardMsg)
		...
		this.fullMsgIdByQueryListenerId.set(listenerId, msgId)
		return listener
	}
	...
	unregisterListener(listenerId) {
		const msgId = this.fullMsgIdByQueryListenerId.get(listenerId)
		...
		if (msgId) {
			...
			this.fullMsgIdByQueryListenerId.delete(listenerId)
			const res = queryListenerById.delete(listenerId)
			...
			const res = msgListenerById.delete(listenerId)
	...
	async handleCallbackQuery(query) {
		const msgId = this.getFullMsgId(query.message)
		if (!this.queryListenerByIdByMsgIdentifier.has(msgId)) {
			return
		}
		const queryListenerById = this.queryListenerByIdByMsgIdentifier.get(msgId)
