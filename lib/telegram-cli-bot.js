...
export default class TelegramCliBot extends TelegramBot {
	...
	constructor(token, opts = {}) {
		...
		this.on('message', async msg => {
			if (msg.text?.startsWith('/')) {
				...
				await this.handleSessionMsg(msg)
			}
		})
	...
	async sendCommandInfos(chatIds) {
		const toInfo = (cmd) => {
			...
			msg += ` \\- ${escapeMarkdownV2(cmd.description) ?? 'No description provided\\.'}`
			return msg
		}

		const infos = Array.from(this.commandByName.values())
			.filter(cmd => cmd.helpMsgEnabled && cmd.name !== CANCEL_COMMAND_NAME) // put /cancel at the end
			.map(toInfo)
		if (this.commandByName.has(CANCEL_COMMAND_NAME)) {
			infos.push(toInfo(this.commandByName.get(CANCEL_COMMAND_NAME)))
		}
	...
	async handleCancelCommand(msg) {
		...
		if (!session) {
			await this.sendMessage(msg.chat.id, 'No command to cancel\\.')
			return
		}	
		this.sessionByUserId.delete(msg.from.id)
		await this.sendMessage(msg.chat.id, `Command /${session._commandName} cancelled\\.`)
	}
	...
	async handleSessionMsg(msg) {
		const session = this.sessionByUserId.get(msg.from?.id)
		if (!session || !this.commandByName.has(session._commandName)) return
	...
	async handleCommand(msg) {
		...
		try {
			...
			// create new session (do not modify session for /cancel since it requires the previous session)
			if (cmd.name !== CANCEL_COMMAND_NAME) {
