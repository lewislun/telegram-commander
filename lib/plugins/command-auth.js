 ...
 * 
 * @typedef {(chatPermission: string) => Promise<ChatPermission|undefined>} StorageGetter
 * @typedef {(chatPermission: string, val: ChatPermission) => Promise<void>} StorageSetter
 * @typedef {(chatPermission: string) => Promise<void>} StorageRemover
 */
...
export default class CommandAuth extends TelegramCommanderPlugin {
	...
	/** @protected @type {StorageGetter} */				storageGetter = undefined
	/** @protected @type {StorageSetter} */				storageSetter = undefined
	/** @protected @type {StorageRemover} */			storageRemover = undefined
	...
	constructor(groups, opts = {}) {
		...
		for (const group of groups) {
			...
			for (const cmdName of group.commandNames) {
				// cannot add authentication for /cancel command
				if (cmdName === CANCEL_COMMAND_NAME) {
					throw new Error(`/cancel cannot have authentication.`)
				}
		...
		// storage
		if (opts?.storage) {
			this.storageGetter = opts.storage.getter
			this.storageSetter = opts.storage.setter
			this.storageRemover = opts.storage.remover
		} else {
			const inMemoryStorage = new Map()
			this.storageGetter = async name => inMemoryStorage.get(name)
			this.storageSetter = async (name, val) => inMemoryStorage.set(name, val)
			this.storageRemover = async name => inMemoryStorage.delete(name)
		}
	}
	...
	init(bot) {
		...
		this.bot.authorizeCommand = this.authorizeCommand.bind(this)
	...
	async authorizeCommand(msg, cmd) {
		...
		if (cmd.name === CANCEL_COMMAND_NAME || group?.noAuth || this.bot.whitelistedChatIdSet?.has(msg.chat.id)) {
			return true
		}

		// return true if: no whitelist and no group (no whitelist = all unspecified commands are allowed)
		if (!this.bot.whitelistedChatIdSet && !group) {
			return true
		}
	...
	async grantChatPermission(chatId, permission, usageCount = -1) {
		if (usageCount === -1) {
			...
			const storageUsageCount = await this.getChatPermission(chatId, permission)
			if (storageUsageCount === undefined) {
				await this.setChatPermission(chatId, permission, usageCount)
			} else {
				await this.setChatPermission(chatId, permission, storageUsageCount + usageCount)
			}
		}
	}
	...
	async getChatPermission(chatId, permission) {
		...
		const val = await this.storageGetter(key)
		return val
	}
	...
	async setChatPermission(chatId, permission, usageCount) {
		const key = this.parseChatPermissionKey(chatId, permission)
		await this.storageSetter(key, {
			usageCount
		})
		const a = await this.storageGetter(key)
	}
