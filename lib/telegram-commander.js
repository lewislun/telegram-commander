...
export default class TelegramCommander extends TelegramBot {
	...
	async handleCommand(msg) {
		...
		try {
			...
			if (!await this.canHandleCommand(cmd, msg)) {
				this.logger?.warn(`Unauthorized command /${cmd.name} from chat ${msg.chat.id}.`)
				await this.sendMessage(msg.chat.id, 'Sorry, you are not authorized to use this command\\.')  // TODO: ability to turn of this reply
				return
			}
