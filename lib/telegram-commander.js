	...
	async handleCommand(msg) {
		...
		try {
			...
			if (!await this.authorizeCommand(msg, cmd)) {
				...
				await this.bot.sendMessage(msg.chat.id, 'Sorry, you are not authorized to use this command\\.')  // TODO: ability to turn off this reply
				return
			}
			...
			if (err instanceof types.ContextCancelError) {
				...
				if (this.errorReplyEnabled) {
					await this.bot.sendMessage(msg.chat.id, escapeMarkdownV2(`Error occurred while handling command /${cmd.name}: ${err}`), { parse_mode: 'MarkdownV2' })
				}
			}
		}
