...
export default class TelegramCommander {
	/** @type {import('node-telegram-bot-api')} */							bot = undefined
	...
	async handleCommand(msg) {
		...
		try {
			...
			if (!await this.authorizeCommand(msg, cmd)) {
				...
				await this.sendMessage(msg.chat.id, 'Sorry, you are not authorized to use this command\\.', { parse_mode: 'MarkdownV2' })  // TODO: ability to turn off this reply
				return
			}
	...
	async sendMessage(chatIds, content, opts = {}) {
		opts = opts ?? {}
		opts.parse_mode = opts.parse_mode ?? this.defaultParseMode
