...
export default class TelegramCommander extends TelegramBot {
	...
	/** @type {((msg: Message, cmd: types.Command) => Promise<void>)[]} */	beforeCommandHandlers = []
	...
	async handleCommand(msg) {
		...
		let context
		try {
			// call beforeCommandHandlers
			for (const handler of this.beforeCommandHandlers) {
				await handler(msg, cmd)
			}

			// ensure the command is permitted
			if (!this.canHandleCommand(msg, cmd)) {
				this.logger?.warn(`Unauthorized command /${cmd.name} from chat ${msg.chat.id}.`)
				await this.sendMessage(msg.chat.id, 'Sorry, you are not authorized to use this command\\.')  // TODO: ability to turn off this reply
				return
			}
