 ...
 * @property {boolean} [isManualInputEnabled=true] If false, text input will be ignored (only inline keyboard input will be accepted).
...
export default class Context {
	...
	async prompt(content, opts = {}) {
		// default options
		const isManualInputEnabled = opts?.isManualInputEnabled ?? true
		const errorMsgFunc = opts?.errorMsg ?? e('Please enter a valid input.')
		const validator = opts?.validator ?? ((result) => !!result)
		const promptTextOnDone = opts?.promptTextOnDone

		const promptMsg = await this.reply(content, opts)
		let result = undefined
		do {
			const promises = [this.waitForCallbackQueryOnce(promptMsg)]
			if (isManualInputEnabled) {
				promises.push(this.waitForMessage())
			}
			const msgOrQuery = await Promise.race(promises)
			result = msgOrQuery?.text ?? msgOrQuery?.data
			if (!validator(result)) {
				const errorMsg = typeof errorMsgFunc === 'function' ? errorMsgFunc(result) : errorMsgFunc
		...
		const promises = [
			...
			this.commander.bot.editMessageReplyMarkup({}, { chat_id: this.chatId, message_id: promptMsg.message_id }),
		]
		if (promptTextOnDone) {
			const newPromptText = promptTextOnDone(result)
			promises.push(this.commander.bot.editMessageText(newPromptText, {
				chat_id: this.chatId,
