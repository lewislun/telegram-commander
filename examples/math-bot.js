...
bot.addCommand({
	...
	handler: async ctx => {
		const question = escapeMarkdownV2('What is 30 + 12?')
		const inlineKeyboardMsg = await ctx.reply(question, { reply_markup: {
			inline_keyboard: [
				[
					{ text: '-3.14', callback_data: '-3.14' },
					{ text: '4 + 3i', callback_data: '4 + 3i' },
					{ text: '42', callback_data: '42' },
				],
				[
					{ text: 'dy/dx', callback_data: 'dy/dx' },
					{ text: '666', callback_data: '666' },
					{ text: 'F = ma', callback_data: 'F = ma' },
				],
			]
		}})
		const query = await ctx.waitForCallbackQueryOnce(inlineKeyboardMsg)
		await ctx.reply(query.data === '42'? 'Correct' : 'Wrong')
	}
})
